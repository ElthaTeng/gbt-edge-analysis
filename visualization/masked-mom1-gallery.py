...
cmapi = plt.cm.get_cmap('Spectral_r').copy()  # mymap  #
cmapi.set_bad(color='black')
...
for i, row in enumerate(T):
    ...
    #     continue
    # else:
    filepath    = os.path.join(datadir, row['Galaxy'] + '_12CO_mom1_' + row['Mask'] + '_se' + row['Session'] + '.fits')  #_noR25
    emom0 = fits.open('maps/'+row['Galaxy']+'_12CO_emom0_'+row['Mask']+'_se'+row['Session']+'.fits')[0].data  #_noR25

    ## Uncomment this if running "noR25" version of mom0 map gallery
    if 'Mrk1418' in row['Galaxy']:
        filepath    = os.path.join(datadir, row['Galaxy'] + '_12CO_mom1_' + row['Mask'] + '_se' + row['Session'] + '.fits')
    ...
    vmini = np.nanpercentile(mapdata, 5)
    ...
    # if row['Galaxy'] in ['NGC0169', 'NGC0495', 'UGC8909', 'CGCG536-030', 'NGC5954', 'UGC09777']:  
    #     vmaxi = 70
    # else:
    vmaxi = np.nanpercentile(mapdata, 95) 
    ...
    #ax.contourf(mask_r25, levels=[-0.5, 0.5], hatches=['xxx', ''], alpha=0, colors='white')
...
cmap = mpl.colormaps['Spectral_r']  # mymap  #
norm = mpl.colors.Normalize(vmin=0.05, vmax=0.95)
...
#####################
### save the plot
savename = os.path.join(plotdir, 'masked_mom1_gallery')
...
fig.savefig(savename + '_datapref_erik_v2_fix62_color.pdf', dpi = 300)
