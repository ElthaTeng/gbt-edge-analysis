...
cmapi.set_bad(color='black')  #dimgray
...
for i, row in enumerate(T):
    ...
    #     continue
    # else:
    filepath    = os.path.join(datadir, row['Galaxy'] + '_12CO_vdisp_' + row['Mask'] + '_se' + row['Session'] + '.fits')  #_noR25
    mom0 = fits.open('maps/'+row['Galaxy']+'_12CO_mom0_'+row['Mask']+'_se'+row['Session']+'.fits')[0].data  #_noR25
    ...
    if 'Mrk1418' in row['Galaxy']:
        filepath    = os.path.join(datadir, row['Galaxy'] + '_12CO_vdisp_' + row['Mask'] + '_se' + row['Session'] + '.fits')
    ...
    vmini = 10  #np.nanpercentile(mapdata, 1)
    ...
    #     vmaxi = 200
    # else:
    vmaxi = 150  #40  #np.nanpercentile(mapdata, 99.9) 
...
#####################
### save the plot
savename = os.path.join(plotdir, 'masked_mom2_gallery')
...
fig.savefig(savename + '_datapref_erik_v2_fix62_max150.pdf', dpi = 300) #_noR25
