...
datadir = './maps/pplsquare_erik_v2/'  ### directory of the data  ./maps/pplsquare_erik_v2/
...
cmapi = plt.cm.get_cmap('magma').copy()
cmapi.set_bad(color='gray')
...
for i, row in enumerate(T):
    ...
    #     continue
    # else:
    filepath    = os.path.join(datadir, row['Galaxy'] + '_12CO_Tpeak_se' + row['Session'] + '_specsm6.fits')  #_specsm4
    ...
    # if row['Galaxy'] in ['NGC0169', 'NGC1056', 'NGC3057', 'UGC02222']:  
    #     vmaxi = 45
    # else:
    vmaxi = 15 #np.nanpercentile(mapdata, 99.9)
    ...
    #mask_r25 = fits.open('maps/'+row['Galaxy']+'_mask_R25_cutout2d.fits')[0].data
    ...
    cornerleftx = 80*u.arcsec # 80 if include all GBT GOV; 45 if squarecut
...
cmap = mpl.cm.magma
norm = mpl.colors.Normalize(vmin=1, vmax=15) #vmin=0.2, vmax=0.999
...
ell = Ellipse((10,60), width = 10, height = 10, angle = (0), edgecolor = 'c', facecolor = 'none', alpha = 1, lw=2.5)
axleg.add_patch(ell)
axleg.text(25,57, s= r'R$_{25} \times$FoV', fontweight='bold', fontsize=12)

# ell2 = Ellipse((10,60), width = 10, height = 10, angle = (0), edgecolor = 'silver', facecolor = 'none', alpha = 1, lw=2.5)
# axleg.add_patch(ell2)
# axleg.text(25,57, s= 'S/N > 3', fontweight='bold', fontsize=12)


#####################
### save the plot
savename = os.path.join(plotdir, 'tpeak_gallery_portrait')
...
fig.savefig(savename + '_erik_v2_fix62_specsm6_color_divbyrms_max15.pdf', dpi = 300) #_min0.1
